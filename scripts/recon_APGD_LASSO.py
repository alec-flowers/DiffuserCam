...
```bash
python scripts/recon_APGD_LASSO.py --psf_fp data/psf/diffcam_rgb.png \
...
from pycsou.linop.conv import Convolve2D, Convolve1D
from pycsou.func.loss import SquaredL2Loss
from pycsou.func.penalty import SquaredL2Norm, L2Norm, L1Norm
...
def reconstruction(
    ...
    #lambda_ = 0.01;                                                                                           #Optimisation parameter
    ...
    lambda_ = 0.1 * np.max(np.abs(F.gradient(0 * data.flatten()  ) ) )
...
if __name__ == "__main__":
    ...
    psf_fp = '../data/psf/diffcam_rgb.png'
    data_fp = '../data/raw_data/thumbs_up_rgb.png'
    n_iter = 50
    gray = True
   
    reconstruction(psf_fp,data_fp, n_iter, gray)
